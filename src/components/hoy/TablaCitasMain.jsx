...
export default function TablaCitasMain({
  ...
  return (
    <Box
      my={"1rem"}
      h={citas && citas.length > 0 ? "50vh" : "initial"}
      id={"AG-Table"}
      w={"100%"}
    >
      ...
      {!citas && (
        <HStack w={"100%"} justifyContent={"center"} py={"2rem"}>
          <Spinner color={"pink.500"} size={"md"} />
        </HStack>
      )}
      {citas && citas.length > 0 && (
        <AgGridReact
          ...
          rowClassRules={{
            "cita-pendiente": ({ data }) =>
              data.en_servicio != 1 && data.pagado != 1 && data.status == 1,
            "cita-confirmada": ({ data }) =>
              data.en_servicio != 1 && data.pagado != 1 && data.status == 2,
            "cita-pagada": ({ data }) =>
              data.en_servicio != 1 && data.pagado == 1,
            "cita-en-servicio": ({ data }) => data.en_servicio == 1,
          }}
...
function getColumnDefinitions(
  ...
  return [
    ...
    },
    {
      ...
      width: 100,
      cellRenderer: ({ data }) => (
        <StatusBadge status={data.status} pagado={data.pagado} />
      ),
    },
    {
      headerName: "Costo",
      ...
      cellRenderer: ({ data, value }) => (
        <CostoSelector data={data} value={value} servicios={servicios} />
      ),
      // valueGetter: (params) => "--",
    ...
    },
    {
      ...
      cellRenderer: ({ data }) => (
        <EnServicioCell data={data} updateEnServicio={updateEnServicio} />
      ),
      width: 100,
      cellStyle: {
        justifyContent: "center",
...
function HorarioCell({ data }) {
  return (
    <HStack justifyContent="center" gap={1.5}>
      <Text fontWeight="bold" color={"blue.600"}>
        {data.hora}
      </Text>
      <Text fontSize={"0.8rem"}>{` a ${addMinutesToTime(
        data.hora,
        data.minutos,
      )}`}</Text>
    </HStack>
...
function EnServicioCell({ data, updateEnServicio }) {
  ...
  const onCheckedChange = async (e) => {
    setLoading(true);
    const resp = await updateEnServicio(data.cita_ID, e.checked ? 1 : 0);
    setLoading(false);
  };
