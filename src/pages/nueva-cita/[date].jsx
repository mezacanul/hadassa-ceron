...
import { formatFechaDMY, getDateObject } from "@/utils/main";
...
export default function NuevaCita() {
  ...
  useEffect(() => {
    ...
    if (currentCita.servicio && currentCita.lashista && !currentCita.horario) {
        ...
        currentCita.clienta ? null : (
          <VStack style={{ width: "65%" }} align={"start"}>
            ...
            <Box w={"100%"} maxH={"65vh"} overflowY={"scroll"}>
              ...
              {lashistas && currentCita.servicio && !currentCita.lashista && (
                <SelectLashistas
                  lashistas={lashistas}
...
function SelectLashistas({ lashistas, selectedDate }) {
  ...
  if (totalDisponibles != null && totalDisponibles == 0) {
    return (
      <HStack justify={"center"} h={"30vh"} align={"center"} w={"100%"}>
        <Heading textAlign={"center"}>{"No hay lashistas disponibles"}</Heading>
      </HStack>
    );
  }
      ...
      {disponibles &&
        ...
        lashistas.map((lashista, i) => {
          return (
            <GridItem
              key={lashista.id}
              display={disponibles[i] > 0 ? "initial" : "none"}
            >
...
function SelectServicios({ servicios }) {
  ...
  return (
    <>
      {!servicios && <Spinner color="pink.500" borderWidth="4px" size={"xl"} />}
        ...
        {servicios &&
          servicios
            .filter((servicio) =>
              servicio.servicio
                .toLowerCase()
                .includes(searchServicio.toLowerCase()),
            )
            .map((servicio) => {
              return (
...
function SelectHorarios({ selectedDate }) {
  ...
  return (
    <HStack justify={"center"} h={"50vh"} align={"center"} w={"100%"}>
      {!horarios && <Spinner color="pink.500" borderWidth="4px" size={"lg"} />}
      ...
      {horarios && horarios.length > 0 && (
        <Grid gridTemplateColumns={"repeat(4, 1fr)"} gap={"1.5rem"} w={"75%"}>
          {horarios.map((hr) => {
            return (
              <Button
                size={"lg"}
  ...
  return (
    <Card.Root overflow="hidden" size="sm" shadow={"md"}>
      ...
      <Box>
        <Card.Body>
          <Card.Title mb="2" color={"pink.600"}>
            ...
            {data.tipo.includes("combo") && (
              <Badge size={"md"} ms={"0.5rem"} colorPalette={"purple"}>
                Combo
              </Badge>
            )}
        ...
        <Card.Footer justifyContent={"space-between"} mt="4" alignItems={"end"}>
          <HStack alignItems={"end"}>
            ...
            <Badge colorPalette={"blue"}>{data.minutos} minutos</Badge>
          </HStack>
          <Button
            size={"sm"}
            bg={"pink.500"}
