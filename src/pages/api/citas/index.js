    ...
    if (req.method === "GET") {
      ...
      let [eventos] = await pool.query(
        `SELECT 
                ...
                WHERE 
                    fecha_init = '${formatFechaYMD(cita.fecha)}'
      ...
      if (eventos.length > 0) {
        ...
        switch (eventos[0].tipo) {
          case "cambio-horario":
            horarioLashistaArray = horarioJSONToFullArray(eventoHorarios);
            break;
          case "dia-libre":
            res.status(200).json([]);
            return;
      ...
      if (POST_Data.action == "getHorariosDisponibles" && req.body.dev) {
        ...
        if (eventos.length > 0) {
          ...
          if (evento.tipo != "cambio-horario") {
            const eventSlots = getEventSlots(evento.hora, evento.minutos);
            ...
            availableArr = availableArr.filter((available) => {
              return !eventSlotsBackwards.includes(available.hora);
            });
          }
        }
      ...
      } else if (POST_Data.action == "getHorariosDisponibles") {
        const available = getAvailable(
          horariosDispPorCama,
          cita,
          horarioDelDia,
...
function formatEventosForAvailableCalculation(eventos, lashista, fecha) {
  const dayNumber = getDayIndexNumber(fecha);
  const horario = getHorarioByDayNumber(lashista, dayNumber);
